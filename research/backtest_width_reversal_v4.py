...
def generate_groups(df):
    ...
    B = valid[
        (valid['expansion_days'] == 3) &
        (valid['exp_start_pre_breadth'] <= 0.30) &
        (valid['exp_start_pre_breadth'].notna())
    ].copy()
...
def main():
    ...
    for horizon in [5, 10, 20]:
        ...
        metrics.append(calc_metrics(B[col], f'B 固定30%'))
    ...
    for horizon in [5, 10, 20]:
        ...
        if len(a_ex) > 0 and len(b_ex) > 0:
            ...
            print(f"  A vs B (动态阈值): Δ={delta_ab:+.2f}%, t={t_ab:+.2f}, p={p_ab:.3f}")
