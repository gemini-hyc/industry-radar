...
纯唤醒条件回测 — 后续20日上涨概率
====================================
当某行业同时满足4个唤醒条件时，统计后续N日（5/10/20）的绝对涨幅和相对超额胜率。

唤醒条件（与 industry_tracker.py 一致）：
  C1 沉睡: breadth 20日百分位 < 0.45
  C2 跳升: breadth 比前1日跳升 > 12pp (0.12)
  C3 涨幅: avg_pct > 1.5%
...
for _, evt in wake_events.iterrows():
    ...
    for h in HOLD_DAYS:
        ...
        results.append({
            ...
            "bw_delta": round(evt["bw_delta"] * 100, 1),  # pp
...
print("唤醒条件回测结果")
print("=" * 100)
print(f"条件: C1(bw20_pct<{WAKE_SLEEP*100:.0f}%) + C2(跳升>{WAKE_DELTA*100:.0f}pp) + "
      f"C3(avg_pct>{WAKE_PCT}%) + C4(放量>{WAKE_AMT_RATIO}x)")
...
# 按唤醒强度分
print("\n── 按唤醒强度（S0分位） ──")
df_results["S0_approx"] = (
    df_results["bw20_pct"].apply(lambda x: 25 if x<5 else 20 if x<10 else 15 if x<20 else 10 if x<35 else 5 if x<45 else 0) +
    df_results["bw_delta"].apply(lambda x: 25 if x>30 else 20 if x>20 else 15 if x>15 else 10 if x>12 else 0) +
    df_results["avg_pct"].apply(lambda x: 25 if x>5 else 20 if x>3 else 15 if x>2 else 10 if x>1.5 else 0)
    # C4 在 results 里是 amt_ratio，用同一列
)

# 按 S0 三分位分
if len(df_results) > 10:
    q33 = df_results["S0_approx"].quantile(0.33)
    q66 = df_results["S0_approx"].quantile(0.66)
    for label, mask in [("弱唤醒(S0低)", df_results["S0_approx"] <= q33),
                         ("中唤醒(S0中)", (df_results["S0_approx"] > q33) & (df_results["S0_approx"] <= q66)),
                         ("强唤醒(S0高)", df_results["S0_approx"] > q66)]:
        sub = df_results[mask]
        print(f"\n  {label} (S0≤{q33:.0f} / ≤{q66:.0f} / >{q66:.0f}, n={len(sub[sub['horizon_target']==5])}):")
        for line in stats_block(sub, label):
            print(line)
