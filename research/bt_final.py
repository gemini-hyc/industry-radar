...
print(f"信号N={len(S)}, 得分分布:"); print(S['score'].value_counts().sort_index())

# §四 分层
print("\n"+"="*60+"\n§四 冷区信号按前20日均宽分层\n"+"="*60)
xk4 = {'<10%':(280,9.06,84),'10-20%':(1499,3.41,68),'20-30%':(2507,2.46,65)}
for lb,lo,hi in [('<10%',0,0.10),('10-20%',0.10,0.20),('20-30%',0.20,0.31)]:
    sub=S[(S['pb20']>=lo)&(S['pb20']<hi)]
    if len(sub)==0: continue
    e=sub['fwd20'].dropna(); w=sub['win20'].mean()
    t=e.mean()/(e.std()/np.sqrt(len(e))) if len(e)>1 else 0
    xn,xe,xw=xk4[lb]
    print(f"  {lb}: N={len(sub)}(小克{xn}), 超额={e.mean():.2f}%(小克{xe:.2f}%), 胜率={w:.1%}(小克{xw}%), t={t:.2f}")
...
for lb,sub,xn,xe,xw in [
    ...
    if len(sub)==0: continue
    e=sub['fwd20'].dropna(); w=sub['win20'].mean()
    print(f"  {lb}: N={len(sub)}(小克{xn}), 超额={e.mean():.2f}%(小克{xe:.2f}%), 胜率={w:.1%}(小克{xw}%)")
...
for th,xn_d,xe,xw,xir in [(3,1.1,1.65,62,1.15),(5,0.4,3.26,67,1.54),(7,0.1,6.71,74,2.43)]:
    ...
    print(f"  ≥{th}: N={len(sub)}, 日均{len(sub)/nd:.2f}(小克{xn_d}), 超额={e.mean():.2f}%(小克{xe}%), 胜率={w:.1%}(小克{xw}%), IR={ir:.2f}(小克{xir})")
...
for lb,m in [('A:纯扩张≥3',idf['exp_d']>=3),('B:纯冷区≤30%',idf['cold']),('C:冷区+扩张',idf['sig']),('D:热区+扩张',(idf['pb20']>0.70)&(idf['exp_d']>=3))]:
    ...
    t=e.mean()/(e.std()/np.sqrt(len(e))) if len(e)>1 else 0
    print(f"  {lb}: N={len(g)}, 超额={e.mean():.2f}%, 胜率={w:.1%}, t={t:.2f}")
...
# 三阶段
print("\n"+"="*60+"\n三阶段稳定性\n"+"="*60)
for lb,m in [('P1结构牛',S['date'].apply(lambda x:'2020'<=str(x)[:4]<='2021')),('P2震荡熊',S['date'].apply(lambda x:'2022'<=str(x)[:4]<='2024')),('P3政策牛',S['date'].apply(lambda x:str(x)[:4]>='2024'))]:
    sub=S[m]; e=sub['fwd20'].dropna(); w=sub['win20'].mean()
    print(f"  {lb}: N={len(sub)}, 超额={e.mean():.2f}%, 胜率={w:.1%}")
...
for lb,c in [('减速','dec'),('加速','acc')]:
    ...
    print(f"  全样本{lb}: N={len(sub)}, 超额={e.mean():.2f}%, 胜率={w:.1%}")
ce3=e3[e3['sig']]
...
for lb,sub in [('冷区+扩张',ce),('冷区无扩张',cn)]:
    e=sub['fwd20'].dropna(); w=sub['win20'].mean()
    t=e.mean()/(e.std()/np.sqrt(len(e))) if len(e)>1 else 0
    print(f"  {lb}: N={len(sub)}, 超额={e.mean():.2f}%, 胜率={w:.1%}, t={t:.2f}")
if len(ce)>0 and len(cn)>0:
    diff=ce['fwd20'].mean()-cn['fwd20'].mean()
    print(f"  扩张增量: {diff:.2f}% ({'>0→宽度反转成立' if diff>0 else '≈0或<0→均值回归为主'})")
...
for lb,m in [('急跌(<-3%)',hot['avg_pct']<-3),('5日跌(<-2%)',hot.groupby('con_code')['avg_pct'].transform(lambda x:x.rolling(5,min_periods=5).sum())<-2)]:
    ...
    print(f"  热区{lb}: N={len(sub)}, 超额={e.mean():.2f}%, 胜率={w:.1%}")
